...
Бот и мобильный API запускаются на сервере **5.101.179.47**. В `.env` на этом сервере добавьте:
...
### Если адрес не открывается на сервере (например http://5.101.179.47:8765/)

1. **Ключ API должен быть задан.**  
   В `.env` обязательно есть `MOBILE_API_KEY=...` или `ALLOWED_USER_ID=...`. Без этого мобильный API не запускается. В логе при старте бота должно быть: `Mobile API: http://0.0.0.0:8765 ...`. Если видите `Mobile API не запущен: задайте MOBILE_API_KEY...` — добавьте переменную и перезапустите бота.

2. **Откройте порт 8765 в файрволе на сервере.**  
   Пример для **ufw**:
   ```bash
   ...
   sudo ufw status
   ```
   Для **firewalld**: `sudo firewall-cmd --add-port=8765/tcp --permanent && sudo firewall-cmd --reload`.  
   В облаке (VPS) откройте порт 8765 в **Security Group / правилах входящего трафика**.

3. **Проверка с самого сервера.**  
   На сервере выполните:
   ```bash
   curl -s http://127.0.0.1:8765/api/health
   ```
   Должен вернуться `{"status":"ok"}`. Если так и есть, но снаружи по `http://IP:8765` не открывается — проблема в файрволе или в облачных правилах.

4. **Проверка снаружи.**  
   В браузере или через `curl`: `http://ВАШ_IP:8765/api/health` — страница без ключа должна отдать JSON `{"status":"ok"}`.

## Настройка приложения

**Сервер бота:** `5.101.179.47`. В приложении указывайте URL: **`http://5.101.179.47:8765`** (без слэша в конце).

1. При первом запуске откройте **Настройки** (иконка шестерёнки).
2. Укажите **URL сервера**:
   - Наш сервер: `http://5.101.179.47:8765`
